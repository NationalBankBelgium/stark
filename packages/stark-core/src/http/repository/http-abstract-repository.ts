...
export abstract class AbstractStarkHttpRepository<T extends StarkResource> {
	...
	public constructor(starkHttpService: StarkHttpService<T>,
		...
		if (typeof resourcePath === "undefined") {
			throw new Error("AbstractStarkHttpRepository: resourcePath missing in Http Repository with backend \"" + this.backend.name + "\".");
		} else {
	...
	public search(criteria: { [param: string]: string },
				  limit: number,
				  offset: number,
				  params?: StarkHttpSearchRequestParams): Observable<StarkCollectionResponseWrapper<T>> {
		return this.starkHttpService.executeCollectionRequest(
			this.getRequestBuilder().search(criteria, limit, offset, params).build()
		);
	}
