...
function isValidBelgianNin(nin: string): boolean {
	...
	const getBirthDate = (value: string): string => value.substring(0, birthdayEndIndex);

	const getOrder = (value: string): string => value.substring(birthdayEndIndex, orderEndIndex);
