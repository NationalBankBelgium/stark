...
export class StarkTimestampMaskDirective
	extends AbstractStarkTextMaskBaseDirective<IMask.MaskedDateOptions, StarkTimestampMaskConfig>
	implements OnChanges
{
	...
	/**
	 * Maximum date for the timestamp mask
	 *
	...
	/**
	 * Minimum date for the timestamp mask
	 *
	 * Supported types: `Date | moment.Moment | undefined | null`
	 */
	@Input()
	...
	public override ngOnChanges(changes: SimpleChanges): void {
		...
		if (this.rebuildMaskNgOnChanges(changes) && this._value && this.maskRef && this.maskConfig) {
			const config = this.mergeMaskConfig(this.maskConfig, this.defaultMask);
			if (config && config.formatFn) {
				this.maskRef.value = config.formatFn(this._value);
			}
		}
	}
	...
	protected override normalizeMaskConfig(maskConfig: string | StarkTimestampMaskConfig): IMask.MaskedDateOptions {
		const mask: StarkTimestampMaskConfig = this.mergeMaskConfig(maskConfig, this.defaultMask);
	...
	private minDateForMask(mask: StarkTimestampMaskConfig): Date | undefined {
		...
		if (this.min) {
			value = this.min.toDate();
		} else if (mask.minDate) {
			if (moment.isMoment(mask.minDate)) {
				value = mask.minDate.toDate();
			} else {
				value = mask.minDate;
			}
		}
		if (value && mask.format.indexOf("H") === -1) {
	...
	private maxDateForMask(mask: StarkTimestampMaskConfig): Date | undefined {
		let value: Date | undefined;
		if (this.max) {
			value = this.max.toDate();
		} else if (mask.maxDate) {
			if (moment.isMoment(mask.maxDate)) {
				value = mask.maxDate.toDate();
			} else {
				value = mask.maxDate;
			}
		}
